...
public class Application {
    ...
    private void run() throws Exception {
        ...
        // contract to the blockchain. Unfortunately not all types can be logged in their entirety.
        // For Strings and arrays, the hash of values is provided, not the original value.
        // For further information, refer to https://docs.web3j.io/filters.html#filters-and-events
        for (Greeter.ModifiedEventResponse event : contract.getModifiedEvents(transactionReceipt)) {
            log.info("Modify event fired, previous value: " + Numeric.toHexString(event.oldGreeting)
                    + ", new value: " + Numeric.toHexString(event.newGreeting));
        }
    }
}
